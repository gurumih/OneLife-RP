...
function Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    for pos, data in ipairs(queueList) do
        if tmp.priority then
            ...
            if _pos then
                Queue:DebugPrint(string_format("%s[%s] otrzymał priorytet i został umieszczony %d/%d w kolejce", tmp.name, ids[1], _pos, queueCount))
                break
            end
        end
    end
    ...
    if not _pos then
        ...
        Queue:DebugPrint(string_format("%s[%s] został umieszczony %d/%d w kolejce", tmp.name, ids[1], _pos, queueCount))
    end
...
function Queue:AddToConnecting(ids, ignorePos, autoRemove, done)
    local function remove()
        ...
        Queue:DebugPrint("Nie można dodać gracza do listy połączeń")
    end
    ...
    if type(id) == "table" then
        for _id, power in pairs(id) do
            if _id and type(_id) == "string" and power and type(power) == "number" then
                ...
                Queue:DebugPrint("Błąd podczas dodawania identyfikatora priorytetu, przekazano nieprawidłowe dane")
                return false
            end
        end
...
local function playerConnect(name, setKickReason, deferrals)
    ...
    if not ids then
        ...
        CancelEvent()
        Queue:DebugPrint("Dropped " .. name .. ", nie udało się odzyskać żadnego z ich identyfikatorów")
        return
    end
    ...
    Queue:CanJoin(src, function(reason)
        ...
        if reason then
            ...
            Queue:RemoveFromQueue(ids)
            Queue:RemoveFromConnecting(ids)
            Queue:DebugPrint(string_format("%s[%s] został zablokowany przed dołączeniem; Powód: %s", name, ids[1], reason))
            CancelEvent()
            return
        end
    ...
    if Queue:IsInQueue(ids) then
        ...
        Queue:DebugPrint(string_format("%s[%s] ponownie dołączył do kolejki po anulowaniu", name, ids[1]))
    else
        Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    if Queue:NotFull(true) and _Queue.JoinDelay <= GetGameTimer() then
        ...
        Queue:DebugPrint(name .. "[" .. ids[1] .. "] ładuje się na serwer")
    ...
    while true do
        ...
        if not data or not data.deferrals or not data.source or not pos then
            remove("[Queue] Usunięto z kolejki, dane kolejki są nieprawidłowe :(")
            Queue:DebugPrint(tostring(name .. "[" .. ids[1] .. "] został usunięty z kolejki, ponieważ zawierał nieprawidłowe dane"))
            return
        end
        ...
        if data.timeout >= Config.QueueTimeOut and os_time() - connectTime > 5 then
            remove("[Queue] Usunięto z powodu przekroczenia limitu czasu")
            Queue:DebugPrint(name .. "[" .. ids[1] .. "] został usunięty z kolejki, ponieważ upłynął limit czasu")
            return
        end
    ...
    while true do
        ...
        while i <= Queue:ConnectingSize() do
            ...
            if ((data.timeout >= 300 and not endPoint) or data.timeout >= Config.ConnectTimeOut) and data.source ~= "debug" and os_time() - data.firstconnect > 5 then
                remove(data)
                Queue:DebugPrint(data.name .. "[" .. data.ids[1] .. "] został usunięty z kolejki połączeń, ponieważ upłynął limit czasu")
            else
                i = i + 1
            end
        end
