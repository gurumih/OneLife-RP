    ...
    if keyboard ~= nil then
        ...
        QBCore.Functions.TriggerCallback("fivem-appearance:server:getOutfits", function(outfits)
            ...
            if outfitExists then
                QBCore.Functions.Notify("Strój o tej nazwie już istnieje.", "error")
                return
            end
...
RegisterNetEvent("fivem-appearance:client:OutfitManagementMenu", function(args)
    ...
    local menuItems = {
        {
            header = "👔 | Zarządzać " .. args.type .. " Outfits",
            isMenuHeader = true
        },
        {
            header = "Change Outfit",
            txt = "Wybierz z dowolnego z aktualnie zapisanych "  .. args.type .. " outfits",
            params = {
        ...
        },
        {
            ...
            txt = "Zapisz swój obecny strój jako " .. args.type .. " outfit",
            params = {
...
function OpenMenu(isPedMenu, backEvent, menuType, menuData)
    ...
    local outfitMenuItems = {{
        header = "Zmień Outfit",
        txt = "Wybierz jeden z aktualnie zapisanych strojów",
        params = {
        ...
        header = "Zapisz nowy strój",
        txt = "Zapisz nowy strój, którego będziesz mógł użyć później",
        params = {
            event = "fivem-appearance:client:saveOutfit"
        }
    }, {
        header = "Usuń strój",
        txt = "Tak... Ten też nam się nie podobał",
        params = {
    ...
    if menuType == "default" then
        local header = "Kup ubranie - $" .. Config.ClothingCost
        if isPedMenu then
            header = "Zmień Outfit"
        end
        menuItems[#menuItems + 1] = {
            header = "Sklep z ubraniami",
            icon = "fas fa-shirt",
            isMenuHeader = true -- Set to true to make a nonclickable title
        }
        menuItems[#menuItems + 1] = {
            header = header,
            txt = "Wybierz z szerokiej gamy przedmiotów do noszenia",
            params = {
        ...
        if isPointInside then
            ...
            if jobName == (currentStore.job or currentStore.gang) then
                ...
                if zoneName == 'clothing' then
                    exports['qb-core']:DrawText(prefix .. 'Sklep z ubraniami')
                elseif zoneName == 'barber' then
                    exports['qb-core']:DrawText(prefix .. 'Barber')
                elseif zoneName == 'tattoo' then
                    exports['qb-core']:DrawText(prefix .. 'Tatuażysta')
                elseif zoneName == 'surgeon' then
