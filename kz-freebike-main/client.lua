...
local function callTaxi() -- Put your dispatch code here
    ...
    QBCore.Functions.Notify("Prośba została wysłana do firmy taksówkarskiej, proszę zostań tam, gdzie jesteś.", "success")
    cooldown = true
    Wait(Config.cooldownTime)
    cooldown = false
    else
        QBCore.Functions.Notify("Przepraszamy, nie ma żadnych taksówkarzy na służbie. Spróbuj ponownie później", "error")
    end
    end)
end
			...
			if distance <= 10 then
                ...
                if distance <= 2 then
                   if Config.taxiDispatch == true then 
                    DrawText3D(bike.x, bike.y, bike.z -0.1, "~g~E~w~ - Darmowy rower </br> ~g~H~w~ - zadzwon po taxi ")
                   ...
                   elseif IsControlJustReleased(0, 74) then
                    callTaxi()
                        ...
                        DrawText3D(bike.x, bike.y, bike.z -0.1, "~g~E~w~ - darmowy rower")
                        if IsControlJustReleased(0, 38) then
                         spawnBike()
                        end
                    end
