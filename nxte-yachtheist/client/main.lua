...
local function OnHack1Done(success)
    if success then 
        ...
        QBCore.Functions.Notify('Pomyślnie wyłączono kamery', 'success')
        ...
        QBCore.Functions.Notify('Nie udało ci się wyłączyć kamer!', 'error')
        TriggerServerEvent('QBCore:Server:RemoveItem', Config.Hack1Item, 1, slot, info)
        TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items[Config.Hack1Item], 'remove')
        CallCops()
    end
...
local function OnHack2Done(success)
    if success then 
        ...
        CallCops()
        QBCore.Functions.Notify('Pomyślnie wyłączono blokadę bezpieczeństwa', 'success')
        ...
        CallCops()
        QBCore.Functions.Notify('Nie udało się włamać blokady zabezpieczającej', 'error')
        TriggerServerEvent('QBCore:Server:RemoveItem', Config.Hack2Item, 1, slot, info)
        TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items[Config.Hack2Item], 'remove')
    end
end
        ...
        if CopCount >= Config.MinCop then
            if not isActive then 
                if cash >= Config.HeistPrice then
                    ...
                    QBCore.Functions.Notify('Kupiłeś lokalizacje GPS dla $'..Config.HeistPrice.. '', 'success')
                    TriggerServerEvent('nxte-yacht:server:SetActive', true)
                    TriggerServerEvent('nxte-yacht:server:removemoney', Config.HeistPrice)
                else 
                    QBCore.Functions.Notify('Czy pracuję tutaj z amature? TAK, chcę to w gotówce','error')
                end
            else 
                local random =  math.random(1, 4)
                if random == 1 then
                    QBCore.Functions.Notify('Nikogo nie ma w domu, wróć później', 'error')
