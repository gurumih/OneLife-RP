					...
					row = `	<div class="col col-md-4">
								<div class="card h-100">
								  <div class="card-body text-center mainmenu-subcard" id="menuMyInvoices" style="background-color: #eeeff3; color: #2f3037;">
								    ...
								    <p class="card-text">Moje faktury</p>
								  </div>
								</div>
							</div>
							<div class="col col-md-4">
								<div class="card h-100">
								  <div class="card-body text-center mainmenu-subcard" id="menuSocietyInvoices" style="background-color: #eeeff3; color: #2f3037;">
								    ...
								    <p class="card-text">Firmowe faktury</p>
								  </div>
								</div>
							</div>
							<div class="col col-md-4">
								<div class="card h-100">
								  <div class="card-body text-center mainmenu-subcard" id="menuCreateInvoice" style="background-color: #eeeff3; color: #2f3037;">
								    ...
								    <p class="card-text">Stwórz fakture</p>
								  </div>
								</div>
							</div>
						  `
			...
			if (!windowIsOpened) {
				...
				for(var i=0; i<event.data.invoices.length; i++)
				{
					...
					if (invoices.status == 'paid') {
						tablestatus = '<td class="text-center align-middle"><span class="badge bg-success" style="font-size: 14px;"><i class="fas fa-check-circle"></i> ZAPŁACONE</span></td>';
						modalstatus = '<span class="badge bg-success" style="position: absolute; right: 5%; top: 5%; font-size: 18px;"><i class="fas fa-check-circle"></i> ZAPŁACONE</span>';
						payment_status = `<span style="font-size: 20px; color: #2f3037; text-transform: uppercase; font-weight: 600;">ZAPŁACONE ON: ${invoices.paid_date}</span>`;
					}
					else if (invoices.status == 'unpaid') {
						tablestatus = '<td class="text-center align-middle"><span class="badge bg-danger" style="font-size: 14px;"><i class="fas fa-times-circle"></i> NIE ZAPŁACONE</span></td>';
						modalstatus = '<span class="badge bg-danger" style="position: absolute; right: 5%; top: 5%; font-size: 18px;"><i class="fas fa-times-circle"></i> NIE ZAPŁACONE</span>';
						payment_status = `<button type="button" id="" class="btn btn-blue flex-grow-1 payInvoice" style="border-radius: 10px; flex-basis: 100%;" data-invoiceId="${invoices.id}" data-invoiceMoney="${invoices.invoice_value}" data-bs-dismiss="modal"><i class="fas fa-shopping-bag"></i> Zapłać ${invoices.invoice_value.toLocaleString()}&euro;</button>`;
					}
					else if (invoices.status == 'autopaid') {
						tablestatus = '<td class="text-center align-middle"><span class="badge bg-info" style="font-size: 14px;"><i class="fas fa-clock"></i> AUTOZAPŁATA</span></td>';
						modalstatus = '<span class="badge bg-info" style="position: absolute; right: 5%; top: 5%; font-size: 18px;"><i class="fas fa-clock"></i> AUTOZAPŁATA</span>';
						payment_status = `<span style="font-size: 20px; color: #2f3037; text-transform: uppercase; font-weight: 600;">AUTOZAPŁATA ON: ${invoices.paid_date}</span>`;
					}
					else if (invoices.status == 'cancelled') {
						tablestatus = '<td class="text-center align-middle"><span class="badge bg-secondary" style="font-size: 14px;"><i class="fas fa-minus-circle"></i> ANULOWANO</span></td>';
						modalstatus = '<span class="badge bg-secondary" style="position: absolute; right: 5%; top: 5%; font-size: 18px;"><i class="fas fa-minus-circle"></i> ANULOWANO</span>';
						payment_status = '<span style="font-size: 20px; color: #2f3037; text-transform: uppercase; font-weight: 600;">ANULOWANO</span>'
					}
					...
					row += `
						<tr>
							...
							<td class="text-center align-middle"><button type="button" class="btn btn-blue showInvoice" style="border-radius: 10px; flex-basis: 100%;" data-toggle="modal" data-target="#showInvoiceModal${invoices.id}" data-invoiceId="${invoices.id}""><i class="fas fa-eye"></i> ZOBACZ</button></td>
						</tr>
					`;
									...
									<div class="modal-dialog modal-dialog-centered">
										<div class="modal-content myinvoices_modal-content">
											<div class="modal-body p-4">
												...
												<br>
												<div class="d-flex justify-content-between">
													<span class="" id="" style="color: #2f3037; font-size: 20px; font-weight: 700;">Razem</span> <span class="" id="" style="font-size: 18px;">${Math.round(invoices.invoice_value).toLocaleString()} &euro;</span>
												</div>
												<br>
