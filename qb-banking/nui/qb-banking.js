...
function populateBanking(data)
{
    ...
    if(data.cardInformation !== undefined) {
        if (data.cardInformation.cardLocked == true) {
            ...
            $("#debitCardStatus").html('<div class="card-header">Karta zablokowana</div><div class="card-body">Twoja karta jest obecnie zablokowana.</div><div class="card-footer"><button class="btn btn-primary btn-block" id="unLockCard">Odblokuj/Zablokuj</button></div>');
        } else {
            $("#debitCardStatus").removeClass('bg-danger');
        $("#debitCardStatus").addClass('bg-success');
        $("#debitCardStatus").html('<div class="card-header">Karta odblokowana</div><div class="card-body">Twoja karta jest aktywna.</div><div class="card-footer"><button class="btn btn-primary btn-block" id="lockCard">Zablokuj/Odblokuj</button></div>');
        }
...
$(function() {
    ...
    $(document).on('click','#lockCard',function(){
        $("#debitCardStatus").removeClass('bg-success');
        $("#debitCardStatus").addClass('bg-danger');
        $("#debitCardStatus").html('<div class="card-header">Karta zablokowana</div><div class="card-body">Twoja karta jest ZABLOKOWANA</div><div class="card-footer"><button class="btn btn-primary btn-block" id="unLockCard">Odblokuj/Zablokuj</button></div>');
    ...
    $("#updateNewPin").click(function() {
        ...
        if(newPin !== null && newPin !== undefined && newPin.replace(/[^0-9]/g,"").length === 4) {
            ...
            $("#newPinReqMsg").html('Musisz ustawić cztero-cyfrowy PIN')
        }
    ...
    $("#initiateWithdraw").click(function() {
        ...
        if(amount !== undefined && amount > 0) {
            ...
            $("#withdrawErrorMsg").html('Wystąpił błąd.');
        }
    });
    ...
    $("#initiateWithdrawATM").click(function() {
        ...
        if (currentLimit + parseInt(amount) <= Config.ATMTransLimit) {
            if(amount !== undefined && amount > 0) {
                ...
                $("#withdrawATMError").css({"display":"none"});
                $("#withdrawATMErrorMsg").html('Wypłaciłeś $' + amount + ' z konta');
                currentLimit = currentLimit + parseInt(amount);
            } else {
                // Error doing withdraw
                $("#withdrawATMErrorMsg").removeClass('alert-success').addClass('alert-danger');
                $("#withdrawATMError").css({"display":"block"});
                $("#withdrawATMErrorMsg").html('Wystąpił błąd');
            }
        } else {
            $("#withdrawATMErrorMsg").removeClass('alert-success').addClass('alert-danger');
            $("#withdrawATMError").css({"display":"block"});
            $("#withdrawATMErrorMsg").html('Osiągnąłeś dzienny limit wypłat $' + Config.ATMTransLimit + ' , proszę odwiedzić bank.');
        }
    });
    ...
    $("#initiateDeposit").click(function() {
        ...
        if(amount !== undefined && amount > 0) {
            ...
            $("#depositErrorMsg").html('Wystąpił błąd');
        }
    });
    ...
    $("#processCard").click(function() {
        ...
        if(pinValue !== null && pinValue !== undefined && pinValue.replace(/[^0-9]/g,"").length === 4) {
            ...
            $("#pinCreatorErrorMsg").html('Wystąpił błąd');
        }
    ...
    $("#initiateTransfer").click(function() {
        ...
        if(amount !== undefined && amount !== null && amount > 0 && sortcode !== undefined && sortcode !== null && sortcode > 0 && account !== undefined && account !== null && account > 0) {
            ...
            $("#transferError").css({"display":"block"});
            $("#transferErrorMsg").html('Wystąpił błąd');
        }
    ...
    $("[data-action=ATMwithdraw]").click(function() {
        ...
        if (currentLimit + parseInt(amount) <= Config.ATMTransLimit) {
            ...
            $("#successMessageATM").html('Osiągnąłeś dzienny limit wypłat $' + Config.ATMTransLimit + ' , proszę odwiedzić bank.');
        }
    });
        ...
        if(number == "ENTER") {
            ...
            if (enteredPin !== null && enteredPin !== undefined || enteredPin.replace(/[^0-9]/g,"").length !== 4) {
                    if(clientPin == enteredPin) {
                        ...
                        $("#pinErrorMsg").html('PIN niepoprawny.');
                        $("#pinErrorDiv").css({"display":"block"});
                    }
            } else {
                $("#pinErrorMsg").html('Wpisz cztero-cyfrowy PIN');
                $("#pinErrorDiv").css({"display":"block"});
            }
