...
function openLocker(bankId, lockerId) -- Globally Used
    ...
    if bankId == "paleto" then
        QBCore.Functions.TriggerCallback('QBCore:HasItem', function(hasItem)
            if hasItem then
                ...
                IsDrilling = true
                QBCore.Functions.Progressbar("open_locker_drill", "Włamywanie się do sejfu ..", math.random(18000, 30000), false, true, {
                    ...
                    QBCore.Functions.Notify("Sukces!", "success")
                    IsDrilling = false
                end, function() -- Cancel
                    StopAnimTask(PlayerPedId(), "anim@heists@fleeca_bank@drilling", "drill_straight_idle", 1.0)
                    TriggerServerEvent('qb-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                    DetachEntity(DrillObject, true, true)
                    DeleteObject(DrillObject)
                    QBCore.Functions.Notify("Anulowano..", "error")
                    IsDrilling = false
                end)
                CreateThread(function()
                    while IsDrilling do
                ...
                QBCore.Functions.Notify("Wygląda na to, że zamek jest za silny ..", "error")
                TriggerServerEvent('qb-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
            end
        end, "drill")
    elseif bankId == "pacific" then
        ...
        IsDrilling = true
        QBCore.Functions.Progressbar("open_locker", "Włamywanie się do sejfu ..", math.random(27000, 37000), false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
                            ...
                            if not Config.SmallBanks[closestBank]["isOpened"] then
                                QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
                                    if result then
                                        ...
                                        QBCore.Functions.Progressbar("hack_gate", "Łączenie urządzenia hakerskiego ..", math.random(5000, 10000), false, true, {
                                            disableMovement = true,
                                            disableCarMovement = true,
                                            disableMouse = false,
                                            disableCombat = true,
                                            ...
                                            TriggerEvent("mhacking:start", math.random(6, 7), math.random(25, 30), OnHackDone)
                                        ...
                                        QBCore.Functions.Notify("Brakuje ci przedmiotu ..", "error")
                                    end
                                end, "trojan_usb")
                            else
                                QBCore.Functions.Notify("Bank jest już otwarty ..", "error")
                            end
                        else
                            QBCore.Functions.Notify('Na służbie musi być przynajmniej '..Config.MinimumFleecaPolice, "error")
                        end
                    end
                end
            else
                QBCore.Functions.Notify("Otworzenie drzwi jest teraz nie możliwe", "error", 5500)
            end
        end)
    end
end)
    ...
    if PlayerJob.name == "police" and onDuty then
        ...
        if type == "small" then
            ...
            TriggerEvent('qb-policealerts:client:AddPoliceAlert', {
                timeOut = 10000,
                alertTitle = "Próba napadu na bank",
                coords = {
                    x = coords.x,
                    y = coords.y,
                    z = coords.z,
        ...
        elseif type == "paleto" then
            ...
            TriggerEvent('qb-policealerts:client:AddPoliceAlert', {
                timeOut = 10000,
                alertTitle = "Próba napadu na Blain County Savings",
                coords = {
                    x = coords.x,
                    y = coords.y,
                    z = coords.z,
        ...
        elseif type == "pacific" then
            ...
            TriggerEvent('qb-policealerts:client:AddPoliceAlert', {
                timeOut = 10000,
                alertTitle = "Próba napadu na Pacific Standard Bank",
                coords = {
                    x = coords.x,
                    y = coords.y,
                    z = coords.z,
                                    ...
                                    if lockerDist < 0.5 then
                                        ...
                                        if IsControlJustPressed(0, 38) then
                                            if CurrentCops >= Config.MinimumFleecaPolice then
                                                ...
                                                QBCore.Functions.Notify('Potrzeba przynajmniej '..Config.MinimumFleecaPolice..' policjantów na służbie', "error")
                                            end
                                        end
                                    end
                                end
