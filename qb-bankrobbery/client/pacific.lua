            ...
            if not isBusy then
                if CurrentCops >= Config.MinimumPacificPolice then
                    if not Config.BigBanks["pacific"]["isOpened"] then
                        ...
                        QBCore.Functions.Progressbar("security_pass", "Potwierdzanie ..", math.random(5000, 10000), false, true, {
                    ...
                    QBCore.Functions.Notify('Na służbie musi być '..Config.MinimumPacificPolice..' policjantów', "error")
                end
            else
                QBCore.Functions.Notify("Otworzenie drzwi jest nie możliwe", "error", 5500)
            end
        end)
    end
end)
            ...
            if not isBusy then
                ...
                if dist < 1.5 then
                    if CurrentCops >= Config.MinimumPacificPolice then
                            ...
                            QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
                                if result then
                                    ...
                                    QBCore.Functions.Progressbar("hack_gate", "Łączenie urządzenia hakerskiego ..", math.random(5000, 10000), false, true, {
                                        ...
                                        TriggerEvent("mhacking:start", math.random(5, 9), math.random(25, 30), OnHackPacificDone)
                        ...
                        QBCore.Functions.Notify('Na służbie musi być przynajmniej '..Config.MinimumPacificPolice..' policjantów', "error")
                    end
                end
            else
                QBCore.Functions.Notify("Otworzenie drzwi nie jest możliwe", "error", 5500)
            end
        end)
    end
end)
...
CreateThread(function()
    ...
    local requiredItems2 = {
        ...
        [3] = {name = QBCore.Shared.Items["security_card_02"]["name"], image = QBCore.Shared.Items["security_card_02"]["image"]},
    }
    local requiredItems = {
                        ...
                        if not Config.BigBanks["pacific"]["lockers"][k]["isOpened"] then
                            if lockerDist < 5 then
                                ...
                                if lockerDist < 0.5 then
                                    DrawText3Ds(Config.BigBanks["pacific"]["lockers"][k]["coords"].x, Config.BigBanks["pacific"]["lockers"][k]["coords"].y, Config.BigBanks["pacific"]["lockers"][k]["coords"].z + 0.3, '[E] Otwórz sejf')
