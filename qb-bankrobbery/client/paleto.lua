                            ...
                            if not Config.BigBanks["paleto"]["lockers"][k]["isOpened"] then
                                if lockerDist < 5 then
                                    ...
                                    if lockerDist < 0.5 then
                                        DrawText3Ds(Config.BigBanks["paleto"]["lockers"][k]["coords"].x, Config.BigBanks["paleto"]["lockers"][k]["coords"].y, Config.BigBanks["paleto"]["lockers"][k]["coords"].z + 0.3, '[E] Włam się')
                                        if IsControlJustPressed(0, 38) then
                                            if CurrentCops >= Config.MinimumPaletoPolice then
                                                openLocker("paleto", k)
                                            else
                                                QBCore.Functions.Notify('Potrzeba przynajmniej '..Config.MinimumPaletoPolice..' policjantów na służbie', "error")
                                            end
                                        end
                                    end
                                end
        ...
        QBCore.Functions.TriggerCallback('qb-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                if CurrentCops >= Config.MinimumPaletoPolice then
                    if not Config.BigBanks["paleto"]["isOpened"] then
                        TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                        QBCore.Functions.Progressbar("security_pass", "Potwierdzanie karty..", math.random(5000, 10000), false, true, {
                    ...
                    QBCore.Functions.Notify('Na służbie musi być przynajmniej '..Config.MinimumPaletoPolice..' policjantów', "error")
                end
            else
                QBCore.Functions.Notify("Nie można otworzyć drzwi..", "error", 5500)
            end
        end)
    end
end)