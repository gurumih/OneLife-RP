        ...
        if dist < 1.5 then
            if CurrentCops >= Config.MinimumThermitePolice then
                if not Config.PowerStations[closestStation].hit then
                    ...
                    QBCore.Functions.Notify("Bezpieczniki się spaliły", "error")
                end
            else
                QBCore.Functions.Notify('Potrzeba przynajmniej '..Config.MinimumThermitePolice..' policjantów na służbie', "error")
            end
        end
...
RegisterNUICallback('thermitesuccess', function()
    QBCore.Functions.TriggerCallback("thermite:server:check", function(success)
        if success then
            ...
            local coords = GetEntityCoords(PlayerPedId())
            while time > 0 do
                QBCore.Functions.Notify("Termit wybuchnie za " .. time .. "..")
                Wait(1000)
                time = time - 1
            end
            local randTime = math.random(10000, 15000)
            ...
            if currentStation ~= 0 then
                QBCore.Functions.Notify("Bezpieczniki zostały zniszczone", "success")
                TriggerServerEvent("qb-bankrobbery:server:SetStationStatus", currentStation, true)
            elseif currentGate ~= 0 then
                QBCore.Functions.Notify("Drzwi są otwarte", "success")
