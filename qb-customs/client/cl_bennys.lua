...
local function AllowJob(restrictionData, job)
    ...
    if Config.Debug then print('Odmówiono mu pozwolenia na pracę. ('..job..')') end
    return false
end
...
local function AllowGang(restrictionData, gang)
    ...
    if Config.Debug then print('Odmówiono, że nie wpuściłeś gangu. ('..gang..')') end
    return false
end
...
local function AllowVehicleClass(restrictionData, vehicle)
    ...
    if restrictionData.deniedClasses then
        for _,class in ipairs(restrictionData.deniedClasses) do
            if vehicleClass == class then
                if Config.Debug then print('Odmowa za odmowę klasy pojazdu. ('..vehicleClass..')') end
                return false
            end
        end
    end
    ...
    if Config.Debug then print('Odmówiono, ponieważ nie dopuszczono klasy pojazdu. ('..vehicleClass..')') end
    return false
end
...
function EnterLocation(override)
    ...
    if not canEnter then
        QBCore.Functions.Notify('Tutaj nic nie możesz zrobić!')
        ExitBennys()
        return
    end
    ...
    local welcomeLabel = (locationData and locationData.settings.welcomeLabel) or "Witamy w firmie Benny's Motorworks!"
