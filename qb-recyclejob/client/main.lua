  ...
  if Config.UseTarget then
    ...
    entranceZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['qb-core']:DrawText('[E] Wejdź do magazynu', 'left')
      else
...
local function GetDutyTargetText()
  local text = onDuty and '[E] Wyjście zegara' or '[E] Wyjście zegara'
  return text
end
  ...
  if Config.UseTarget then
    ...
    deliveryZone:onPlayerInOut(function(isPointInside)
      if isPointInside and carryPackage then
        exports['qb-core']:DrawText('[E] Ręka w pakiecie', 'left')
      else
        exports['qb-core']:HideText()
      end
...
function RegisterPickupTarget(coords)
  ...
  if Config.UseTarget then
    ...
    pickupZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['qb-core']:DrawText('[E] Pobierz pakiet', 'left')
      else
        exports['qb-core']:HideText()
      end
...
RegisterNetEvent('qb-recyclejob:client:target:toggleDuty', function()
  onDuty = not onDuty
  if onDuty then
    QBCore.Functions.Notify('Zostałeś taktowany w', 'success')
    GetRandomPackage()
  else
    QBCore.Functions.Notify('Masz wyrejestrowany czas', 'error')
    DestroyPickupTarget()
  end
