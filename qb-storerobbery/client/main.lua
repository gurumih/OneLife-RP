    ...
    while true do
        ...
        for k in pairs(Config.Registers) do
            ...
            if dist <= 1 and Config.Registers[k].robbed then
                inRange = true
                DrawText3Ds(Config.Registers[k][1].xyz, 'Kasa fiskalna jest pusta')
            end
        end
        if not inRange then
            Wait(2000)
                ...
                if dist < 3 then
                    ...
                    if dist < 1.0 then
                        if not Config.Safes[safe].robbed then
                            DrawText3Ds(Config.Safes[safe][1].xyz, '~g~E~w~ - Wypróbuj połączenie')
                            if IsControlJustPressed(0, 38) then
                                if CurrentCops >= Config.MinimumStoreRobberyPolice then
                                    ...
                                    QBCore.Functions.Notify("Nie ma wystarczająco policjantów (".. Config.MinimumStoreRobberyPolice .." Potrzebnych)", "error")
                                end
                            end
                        else
                            DrawText3Ds(Config.Safes[safe][1].xyz, 'Sejf Otworzony')
                        end
                    end
                end
            end
    ...
    for k in pairs(Config.Registers) do
        ...
        if dist <= 1 and not Config.Registers[k].robbed then
            if CurrentCops >= Config.MinimumStoreRobberyPolice then
                ...
                QBCore.Functions.Notify("Nie ma wystarczająco policjantów (2 Potrzeba)", "error")
            end
        end
    end
end)
