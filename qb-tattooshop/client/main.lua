...
Citizen.CreateThread(function()
	AddTextEntry("ParaTattoos", "Studio tatuaży")
...
function RemoveTattoo(name, label)
	...
	QBCore.Functions.Notify("Usunięto " .. GetLabelText(label) .. " z powodzeniem", "success", 4000)
end

function CreateScale(sType)
	if scaleString ~= sType and sType == "OpenShop" then
		scaleType = setupScaleform("instructional_buttons", "Wybierz tatuaż", 38)
		scaleString = sType
	elseif scaleString ~= sType and sType == "Control" then
		scaleType = setupScaleform2("instructional_buttons", "Zmień kamere", 21, "Zmień widoczność", {90, 89}, "Kup/usuń tatuaż", 191)
		scaleString = sType
	end
end

Citizen.CreateThread(function()
	JayMenu.CreateMenu("tattoo", "Studio tatuaży", function()
