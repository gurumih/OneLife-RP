...
RegisterNetEvent('AttackTransport:InfoForLspd', function(x, y, z)
    if PlayerJob ~= nil and PlayerJob.name == 'police' then
        ...
        if PoliceBlip == 0 then
            ...
            AddTextComponentString('Atak na transport gotówki')
        ...
        if PoliceDist <= 4.5 then
            ...
            if SilenceAlarm == 0 then
                hintToDisplay('Naciśnij ~INPUT_DETONATE~, aby wyciszyć alarm')
                SilenceAlarm = 1
            end
...
RegisterNetEvent('qb-armoredtruckheist:client:robberyCall', function(streetLabel, coords)
    if PlayerJob.name == "police" then
        ...
        TriggerEvent('qb-policealerts:client:AddPoliceAlert', {
            timeOut = 10000,
            alertTitle = "Próba napadu na opancerzoną ciężarówkę",
            coords = {
                x = coords.x,
                y = coords.y,
                z = coords.z
...
function MissionNotification()
    ...
    TriggerServerEvent('qb-phone:server:sendNewMail', {
        sender = "The Boss",
        subject = "New Target",
        message = "Więc jesteś zainteresowany zarobieniem pieniędzy? dobrze... idź po broń i spraw, aby to się stało... wyślij ci teraz lokalizację."
    })
    Citizen.Wait(3000)
end
---
    ...
    while true do
        ...
        if MissionStart == 1 then
            ...
            if dist <= 55.0 then
                ...
                if warning == 0 then
                    warning = 1
                    QBCore.Functions.Notify("Pozbądź się strażników, zanim podłożysz bombę.", "error")
                end
            ...
            if dist <= 7 and BlownUp == 0 and PlayerJob.name ~= 'police' then
                if BlowBackdoor == 0 then
                    hintToDisplay('Naciśnij [G], aby wysadzić tylne drzwi i zabrać pieniądze')
                    BlowBackdoor = 1
                end
                if IsControlPressed(0, 47) and GuardsDead == 1 then
    ...
    if IsVehicleStopped(transport) then
        if IsVehicleSeatFree(transport, -1) and IsVehicleSeatFree(transport, 0) and IsVehicleSeatFree(transport, 1) and
            ...
            if not IsEntityInWater(PlayerPedId()) then
                ...
                QBCore.Functions.Notify('Ładunek zostanie zdetonowany w ' .. TimeToBlow / 1000 .. ' seconds.', "error")
                ...
                BlownUp = 1
                lootable = 1
                QBCore.Functions.Notify('Możesz zacząć zbierać gotówkę.', "success")
                RemoveBlip(TruckBlip)
            else
                QBCore.Functions.Notify('Wyjdź z wody', "error")
            end
        else
            QBCore.Functions.Notify('Aby umieścić ładunek, pojazd musi być pusty', "error")
        end
    else
        QBCore.Functions.Notify('Nie możesz obrabować poruszającego się pojazdu.', "error")
    end
end
        ...
        if lootable == 1 then
            ...
            if dist <= 4.5 then
                if PickupMoney == 0 then
                    hintToDisplay('Naciśnij [E], aby wziąć pieniądze')
                    PickupMoney = 1
                end
                if IsControlJustPressed(0, 38) then
                    lootable = 0
...
function TakingMoney()
    ...
    TaskPlayAnim(PlayerPedId(), "anim@heists@ornate_bank@grab_cash_heels", "grab", 8.0, -8.0, -1, 1, 0, false, false,
        false)
    FreezeEntityPosition(PlayerPedId(), true)
    QBCore.Functions.Notify('Pakujesz gotówkę do torby', "success")
    local _time = GetGameTimer()
    while GetGameTimer() - _time < 20000 do
        if IsControlPressed(0, 47) then
            break
        end
        hintToDisplay('Przytrzymaj [G], aby uciec')
        Citizen.Wait(1)
    end
