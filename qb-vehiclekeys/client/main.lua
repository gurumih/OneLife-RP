            ...
            if result then
                ...
                if vehLockStatus == 1 then
                    ...
                    if (GetVehicleDoorLockStatus(veh) == 2) then
                        ...
                        QBCore.Functions.Notify("Zamknięto pojazd")
                    else
                        QBCore.Functions.Notify("Coś poszło nie tak")
                    end
                else
                    Wait(750)
                    ClearPedTasks(ped)
                    ...
                    if (GetVehicleDoorLockStatus(veh) == 1) then
                        ...
                        Wait(200)
                        SetVehicleLights(veh, 0)
                        QBCore.Functions.Notify("Otworzono pojazd")
                    else
                        QBCore.Functions.Notify("Coś poszło nie tak")
                    end
                end
            else
                QBCore.Functions.Notify('Nie masz kluczy do tego pojazdu', 'error')
            end
        end, plate)
    end
end
...
function lockpickFinish(success)
    ...
    if success then
       -- TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
        QBCore.Functions.Notify('Otworzono drzwi', 'success')
        ...
        PoliceCall()
       -- TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
       -- QBCore.Functions.Notify('Ktoś wezwał policję!', 'error')
    end
...
function Hotwire()
    if not HasKey then
        ...
        PoliceCall()
        QBCore.Functions.Progressbar("hotwire_vehicle", "Odpalanie pojazdu", hotwireTime, false, true, {
            ...
            if (math.random() <= Config.HotwireChance) then
                ...
                QBCore.Functions.Notify("Udało się!")
            else
                SetVehicleEngineOn(veh, false, false, true)
                TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
                QBCore.Functions.Notify("Nie udało się", "error")
            end
            IsHotwiring = false
        end, function() -- Cancel
            StopAnimTask(ped, "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 1.0)
        ...
        if math.random() <= chance then
            ...
            if closestPed ~= nil then
                ...
                if IsPedInAnyVehicle(ped) then
                    ...
                    local msg = "Kradzież pojazdu na " .. streetLabel .. ". Pojazd: " .. Name .. ", Rejestracja: " .. modelPlate
                    local alertTitle = "Kradzież pojazdu"
...
RegisterKeyMapping('togglelocks', 'Otwórz/zamknij pojazd', 'keyboard', 'L')
        ...
        if LocalPlayer.state.isLoggedIn then
            ...
            if IsPedInAnyVehicle(ped, false) and lockpicked and not IsHotwiring and not HasKey then
                ...
                if GetPedInVehicleSeat(veh, -1) == PlayerPedId() then
                    DrawText3D(vehpos.x, vehpos.y, vehpos.z, "~g~H~w~ - Odpal pojazd")
