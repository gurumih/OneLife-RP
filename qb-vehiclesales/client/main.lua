...
local function SellToDealer(sellVehData, vehicleHash)
    CreateThread(function()
        ...
        while keepGoing do
            ...
            DrawText3Ds(coords.x, coords.y, coords.z + 1.6, '~g~7~w~ - Potwierdź / ~r~8~w~ - Anuluj ~g~')
...
local function sellVehicleWait(price)
    ...
    Wait(1500)
    DoScreenFadeIn(250)
    QBCore.Functions.Notify('Twój samochód został wystawiony na sprzedaż! - $'..price, 'success')
        ...
        if QBCore ~= nil then
            ...
            if sellBackDist <= 13.0 and IsPedInAnyVehicle(ped) then
                ...
                if sellBackDist <= 3.5 and IsPedInAnyVehicle(ped) then
                    ...
                    DrawText3Ds(Config.SellVehicleBack.x, Config.SellVehicleBack.y, Config.SellVehicleBack.z, '[~g~E~w~] - Sprzedaj pojazd dealerowi za ~g~$'..math.floor(sellVehData.price / 2))
                    if IsControlJustPressed(0, 38) then
                        QBCore.Functions.TriggerCallback('qb-garage:server:checkVehicleOwner', function(owned)
                            if owned then
                                SellToDealer(sellVehData, GetVehiclePedIsIn(ped))
                            else
                                QBCore.Functions.Notify('to nie jest twój pojazd...', 'error', 3500)
                            end
            ...
            if inRange then
                    ...
                    if dstCheck <= 2 then
                        if not IsPedInAnyVehicle(ped) then
                            if not isConfirming then
                                DrawText3Ds(vehPos.x, vehPos.y, vehPos.z + 1.45, '[~g~E~w~] - Wyświetl umowę dotyczącą pojazdu')
                                DrawText3Ds(vehPos.x, vehPos.y, vehPos.z + 1.25, QBCore.Shared.Vehicles[Config.OccasionSlots[i]["model"]]["name"]..', Cena: ~g~$'..Config.OccasionSlots[i]["price"])
                                if Config.OccasionSlots[i]["owner"] == QBCore.Functions.GetPlayerData().citizenid then
                                    DrawText3Ds(vehPos.x, vehPos.y, vehPos.z + 1.05, '[~r~G~w~] - Anuluj sprzedaż pojazdu')
                                ...
                                DrawText3Ds(vehPos.x, vehPos.y, vehPos.z + 1.45, 'Czy na pewno nie chcesz już sprzedawać swojego pojazdu?')
                                DrawText3Ds(vehPos.x, vehPos.y, vehPos.z + 1.25, '[~g~7~w~] - Tak | [~r~8~w~] - Nie')
                                if IsDisabledControlJustPressed(0, 161) then
                                    isConfirming = false
                                    currentVehicle = i
                ...
                if sellDist <= 13.0 and IsPedInAnyVehicle(ped) then
                    ...
                    if sellDist <= 3.5 and IsPedInAnyVehicle(ped) then
                        DrawText3Ds(Config.SellVehicle.x, Config.SellVehicle.y, Config.SellVehicle.z, '[~g~E~w~] - Aby wystawić pojazd na giełdę')
                        if IsControlJustPressed(0, 38) then
