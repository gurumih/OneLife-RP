...
RegisterNetEvent('tc-pmeter-rob', function()
    ...
    QBCore.Functions.TriggerCallback('QBCore:HasItem', function(hasItem)
        if hasItem then
            ...
            if success then
                QBCore.Functions.Progressbar('rob_meter', 'Okradanie parkometrów', 15000, false, true, { -- Name | Label | Time | useWhileDead | canCancel
            ...
            QBCore.Functions.Notify('Wytrych wygięty cipko', 'error', 7500)
            TriggerServerEvent("QBCore:Server:RemoveItem", "lockpick", 1)
            TriggerEvent("inventory:client:ItemBox", QBCore.Shared.Items["lockpick"], "remove")
            policeAlert()
        end
        else
            QBCore.Functions.Notify('Potrzebujesz wytrychu', 'error', 7500)
        end
    end, 'lockpick')
end)
